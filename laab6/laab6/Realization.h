...
void applySpecialCondition(SpecialTransport<T>& transport) {
    cout << "Enter special condition for transport (e.g., 'Expedited' for faster delivery): ";
    string condition;
    cin >> condition;
...
void distributeCargo(Transport<T>* arr, int size, double totalCargo) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        double load = std::min(arr[i].getMaxLoad(), remaining);
...
void createMenu() {
    ...
    do {
        ...
        case 2:
            for (int i = 0; i < size; i++) {
                if (SpecialTransport<T>* specialTransport = dynamic_cast<SpecialTransport<T>*>(&transports[i])) {
                    cout << "For transport #" << i + 1 << ":\n";
                    applySpecialCondition(*specialTransport);
                }
            }
        ...
        case 3:
            ...
            cin >> cargoWeight;
            distributeCargo(transports, size, cargoWeight);
            break;
        case 4:
